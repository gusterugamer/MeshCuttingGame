...
{    public static float IsAPointLeftOfVectorOrOnTheLine(Vector2 a, Vector2 b, Vector2 p)
    ...
    public static List<Vector3> GetConvexHull(List<Vector3> points)
    {
        ...
        //Step 1. Find the Vector3 with the smallest x coordinate
        //If several have the same x coordinate, find the one with the smallest z
        Vector3 startVector3 = points[0];

        Vector3 startPos = startVector3;
        ...
        for (int i = 1; i < points.Count; i++)
        {
            ...
            {
                startVector3 = points[i];

                startPos = startVector3;
            }
        }

        //This Vector3 is always on the convex hull
        convexHull.Add(startVector3);

        points.Remove(startVector3);
