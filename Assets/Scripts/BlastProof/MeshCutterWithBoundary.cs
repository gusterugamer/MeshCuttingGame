...
{
    public class MeshCutterWithBoundary
    {
        ...
        public float Cut(GameObject obj, Utils.Plane plane, bool triangulateHoles, bool deleteOriginal, Vector4 crossSection, out GameObject cut0, out GameObject cut1, out ContourData intersectionData, in List<BoundaryPoint> bp)
        {
            ...
            if (useCentroid)
            {
                ms = CutWithCentroid(meshToCut, obj.transform, plane, triangulateHoles, crossSection, out mesh0, out mesh1, out centroid0, out centroid1, out intersectionData, bp);
            }
            else
            {
                ms = CutWithoutCentroid(meshToCut, obj.transform, plane, triangulateHoles, crossSection, out mesh0, out mesh1, out intersectionData, bp);
            }
        ...
        public ContourData GetIntersectionData(GameObject obj, Utils.Plane plane)
        {
            ...
            CutBase2(meshToCut, obj.transform, plane, true, false, true, true, crossSectionUV, out mesh0, out mesh1, out c0, out c1, out conourList);
        ...
        public float Cut(Mesh mesh, Transform meshTransform, Utils.Plane plane, bool triangulateHoles, Vector4 crossSection, out Mesh mesh0, out Mesh mesh1, in List<BoundaryPoint> bp)
        {
            Vector3 c0, c1;
            ContourData conourList;
            return CutBase(mesh, meshTransform, plane, triangulateHoles, false, true, false, crossSection, out mesh0, out mesh1, out c0, out c1, out conourList, bp);
        }
        ...
        /// <returns>processing time</returns>
        public float CutWithCentroid(Mesh mesh, Transform meshTransform, Utils.Plane plane, bool triangulateHoles, Vector4 crossSection, out Mesh mesh0, out Mesh mesh1, out Vector3 centroid0, out Vector3 centroid1, out ContourData intersectionData, in List<BoundaryPoint> bp)
        {
            return CutBase(mesh, meshTransform, plane, triangulateHoles, true, true, false, crossSection, out mesh0, out mesh1, out centroid0, out centroid1, out intersectionData, bp);
        }
        ...
        /// <returns>processing time</returns>
        public float CutWithoutCentroid(Mesh mesh, Transform meshTransform, Utils.Plane plane, bool triangulateHoles, Vector4 crossSection, out Mesh mesh0, out Mesh mesh1, out ContourData intersectionData, in List<BoundaryPoint> bp)
        {
            Vector3 c0, c1;
            return CutBase(mesh, meshTransform, plane, triangulateHoles, false, true, false, crossSection, out mesh0, out mesh1, out c0, out c1, out intersectionData, bp);
        }
        ...
        float CutBase2(Mesh mesh, Transform meshTransform, Utils.Plane plane, bool triangulateHoles, bool fixPivot, bool getContourList, bool dontCut,
        ...
        float CutBase(Mesh mesh, Transform meshTransform, Utils.Plane plane, bool triangulateHoles, bool fixPivot, bool getContourList, bool dontCut,
                         Vector4 crossSection, out Mesh mesh0, out Mesh mesh1, out Vector3 centroid0, out Vector3 centroid1, out ContourData intersectionData, in List<BoundaryPoint> bp)
        {
            var stopWatch = new Stopwatch();
            stopWatch.Start();
